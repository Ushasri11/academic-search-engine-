package com.academicSearchEngine.search.commons.exceptions;
...
import com.academicSearchEngine.search.cache.CacheContainer;
import com.academicSearchEngine.search.cache.CacheNames;
import com.academicSearchEngine.search.commons.logger.LogManager;
import com.academicSearchEngine.search.commons.logger.Logger;
import com.academicSearchEngine.search.commons.logger.MessageType;
...
public class RestExceptionHandler extends ResponseEntityExceptionHandler {
	...
	protected ResponseEntity<RestResponse<?>> handleEntityNotFoundException(EntityNotFoundException ex,
		...
		sendEmail(LOGGER.getValue(Constant.REQUEST_KEY),
				"RequestKey: " + LOGGER.getValue(Constant.REQUEST_KEY) + "<br>RequestId: "
						+ LOGGER.getValue(Constant.REQUEST_ID) + "<br/>" + ex.getLocalizedMessage()
						+ "<br>StackTrace:<br/>" + ex.getMessage(),
				environment + ": EntityNotFoundException");
	...
	protected ResponseEntity<RestResponse<?>> handleEntityAlreadyExistsException(EntityAlreadyExistsException ex,
		...
		sendEmail(LOGGER.getValue(Constant.REQUEST_KEY),
				"RequestKey: " + LOGGER.getValue(Constant.REQUEST_KEY) + "<br>RequestId: "
						+ LOGGER.getValue(Constant.REQUEST_ID) + "<br/>" + ex.getLocalizedMessage()
						+ "<br>StackTrace:<br/>" + trace,
				environment + ": EntityAlreadyExistsException");
		return RestUtils.errorResponse(ex.getMessage(), ex.getHttpStatus());
	}
	...
	public ResponseEntity<RestResponse<?>> handleMaxSizeException(MaxUploadSizeExceededException exc,
		...
		sendEmail(LOGGER.getValue(Constant.REQUEST_KEY),
				"RequestKey: " + LOGGER.getValue(Constant.REQUEST_KEY) + "<br>RequestId: "
						+ LOGGER.getValue(Constant.REQUEST_ID) + "<br/>" + "File size cannot exceed from " + maxFileSize
						+ "<br>StackTrace:<br/>" + "File size cannot exceed from " + maxFileSize,
				environment + ": MaxUploadSizeExceededException");
		return RestUtils.errorResponse("File size cannot exceed from " + maxFileSize, HttpStatus.PAYLOAD_TOO_LARGE);
	}
	...
	protected ResponseEntity<RestResponse<?>> unexpectedRollbackException(UnexpectedRollbackException ex,
		...
		sendEmail(LOGGER.getValue(Constant.REQUEST_KEY),
				"RequestKey: " + LOGGER.getValue(Constant.REQUEST_KEY) + "<br>RequestId: "
						+ LOGGER.getValue(Constant.REQUEST_ID) + "<br/>" + ex.getLocalizedMessage()
						+ "<br>StackTrace:<br/>" + trace,
				environment + ": UnexpectedRollbackException");
		return RestUtils.errorResponse(ex.getLocalizedMessage(), ex.getLocalizedMessage(),
				HttpStatus.INTERNAL_SERVER_ERROR);
	}

	@ExceptionHandler(PessimisticLockingFailureException.class)
	public ResponseEntity<RestResponse<?>> lockException(PessimisticLockingFailureException ex, WebRequest request) {
		LOGGER.error(ex.getRootCause().getLocalizedMessage(), ex);
		LogManager.error(getClass(), ex.getRootCause().getLocalizedMessage(), MessageType.Error);
		sendEmail(LOGGER.getValue(Constant.REQUEST_KEY),
				"RequestKey: " + LOGGER.getValue(Constant.REQUEST_KEY) + "<br>RequestId: "
						+ LOGGER.getValue(Constant.REQUEST_ID) + "<br>" + ex.getRootCause().getLocalizedMessage(),
				environment + ": PessimisticLockingFailureException");
	...
	private void sendEmail(String requestKey, String message, String subject) {
		CacheContainer.getInstance().addToContainer(requestKey, subject + "<br>" + message,
				CacheNames.exceptionalEmailer.getName());
	}
